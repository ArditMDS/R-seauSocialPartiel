...
if(!isset($_GET['id']) OR !is_numeric($_GET['id']))
    header('Location: test.php');
        ...
        if(!empty($_POST))
        {
            ...
            if(count($errors) == 0)
            {
                $comment = addCommentThread($id, $author,$comment);

                $success = '<div class="center"><h1><?= $article->title ?></h1></div>';
        ...
        $threadPost = getCommThread($id);
        $comments = getCommentsThread($id);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<nav>
    ...
    <div class="center">
        <div class="divBoxTextePostCol">
            <div class="dFlexName"><?php
                 $color = randomColor();
                 echo "<div class='cercleProfile' id='{$threadPost->id}' style='background-color: $color'></div>";
             ?>
            <p><?= $threadPost->nom ?></p>   
            <time><?= $threadPost->date ?></time>
            </div>
            <div><p><?= $threadPost->message ?></p></div>
        </div>
</div>
<?php
    if(isset($success))
    ...
    <div class="paddArticle"> 
            ...
            <form action="thread.php?id=<?= $threadPost->id?>" method="post">
