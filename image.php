...
if(!isset($_GET['id']) OR !is_numeric($_GET['id']))
    header('Location: test.php');
        ...
        if(!empty($_POST))
        {
            ...
            if(count($errors) == 0)
            {
                $comment = addCommentImage($id, $author,$comment);
        ...
        $imagePost = getImages($id);
        $comments = getCommentsImage($id);
    }
?>
...
<nav>
    ...
    <div class="center">
        <h1>Voulez-vous commenter cette photo ?</h1>
    </div>
    <div class="center">
    <div class="divBoxTextePostCol">
            <div class="dFlexName">
                <?php
                    $color = randomColor();
                    echo "<div class='cercleProfile' id='{$imagePost->id}' style='background-color: $color'></div>";
                ?>
                <p><?= $imagePost->imageUrl ?></p>   
                <time><?= $imagePost->date ?></time>
                </div>
                <img src="./images/<?= $imagePost->nom ?>" alt="">
                <p><?= $imagePost->texte ?></p>
        </div>
    </div>
    <?php
    ...
    <div class="paddArticle"> 
            ...
            <form action="image.php?id=<?= $imagePost->id?>" method="post">
